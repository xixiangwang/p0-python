# ===== d4_list.py =====
# 本文件包含：① 最大/最小/平均值（手写循环 + 内置函数各一遍）② list 全量 API（建/取/改/查/增/删/排序）③ 切片（左开右闭 · a[::-1]）④ 三种遍历（for / enumerate / range）⑤ 两个引用坑（赋值≠拷贝、[[0]*3]*3）
# 相关：切片反转 → d5_palindrome.py；负索引 + sort 排序 → d5_wordcount.py
# ========================
